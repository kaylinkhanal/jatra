...
const  Map =({ posix }: MapProps) =>{
  const user = {
    name: "John Doe",
    imageUrl: "/avatar.png",
  };
  const userPosition: [number, number] = [27.700769, 85.30014];
  ...
  const venueIcon = new L.Icon({
    iconUrl:
      "https://www.shutterstock.com/image-vector/red-color-inserted-label-word-260nw-1954191421.jpg",
  ...
  const generateIcon = (isFull: any) => {
    return new L.Icon({
      iconUrl:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSt3m1Nz9EL53Dck61ZJ5dlJPZoWlaIc26anA&s",
      iconSize: [64, 64], // Adjust size as needed
      className: `rounded-full border-4 border-${
        isFull ? "black" : "green-500"
      } shadow-md `,
      iconAnchor: [16, 32], // Adjust anchor if needed
      popupAnchor: [0, -32], // Adjust popup anchor if needed
    });
    ...
    >
      ...
      {eventsList.map((item: any, id: any) => {
        return (
          <Marker
            key={id}
            position={[item.venue.latitude, item.venue.longitude]}
            icon={generateIcon(item.isFull)}
          >
            <Popup>
              {item.title}
              {item.event_type}
